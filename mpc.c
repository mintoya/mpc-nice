...
thread_local AllocatorV mpc_ctx_allocator_ = nullptr;
...
static mpc_input_t *mpc_input_new_string(const char *filename,
                                         const char *string) {
  mpc_input_t *i = mpc_ctx_malloc(sizeof(mpc_input_t));

  i->filename = mpc_ctx_malloc(strlen(filename) + 1);
  strcpy(i->filename, filename);
  i->type = MPC_INPUT_STRING;

  i->state = mpc_state_new();

  i->string = mpc_ctx_malloc(strlen(string) + 1);
  ...
  i->marks = mpc_ctx_malloc(sizeof(mpc_state_t) * i->marks_slots);
  i->lasts = mpc_ctx_malloc(sizeof(char) * i->marks_slots);
  i->last = '\0';
...
static mpc_input_t *mpc_input_new_nstring(const char *filename,
  ...
  i->string = mpc_ctx_malloc(length + 1);
...
static void mpc_input_delete(mpc_input_t *i) {
  ...
  mpc_ctx_free(i->filename);

  if (i->type == MPC_INPUT_STRING) {
    mpc_ctx_free(i->string);
  }
  if (i->type == MPC_INPUT_PIPE) {
    mpc_ctx_free(i->buffer);
  }

  mpc_ctx_free(i->marks);
  mpc_ctx_free(i->lasts);
  mpc_ctx_free(i);
}
...
static void *mpc_export(mpc_input_t *i, void *p) {
  ...
  q = mpc_ctx_malloc(sizeof(mpc_mem_t));
  memcpy(q, p, sizeof(mpc_mem_t));
  mpc_free(i, p);
  return q;
}
...
static void mpc_input_mark(mpc_input_t *i) {
  ...
  if (i->marks_num > i->marks_slots) {
    ...
    i->marks = mpc_ctx_realloc(i->marks, sizeof(mpc_state_t) * i->marks_slots);
    i->lasts = mpc_ctx_realloc(i->lasts, sizeof(char) * i->marks_slots);
  }
...
static int mpc_input_success(mpc_input_t *i, char c, char **o) {
  ...
  if (i->type == MPC_INPUT_PIPE && i->buffer && !mpc_input_buffer_in_range(i)) {
    i->buffer = mpc_ctx_realloc(i->buffer, strlen(i->buffer) + 2);
...
void mpc_err_delete(mpc_err_t *x) {
  ...
  for (i = 0; i < x->expected_num; i++) {
    mpc_ctx_free(x->expected[i]);
  }
  mpc_ctx_free(x->expected);
  mpc_ctx_free(x->filename);
  mpc_ctx_free(x->failure);
  mpc_ctx_free(x);
}
...
char *mpc_err_string(mpc_err_t *x) {
  ...
  int pos = 0;
  int max = 1023;
  char *buffer = mpc_ctx_calloc(1, 1024);
  ...
  return mpc_ctx_realloc(buffer, strlen(buffer) + 1);
}
...
static mpc_err_t *mpc_err_file(const char *filename, const char *failure) {
  mpc_err_t *x;
  x = mpc_ctx_malloc(sizeof(mpc_err_t));
  x->filename = mpc_ctx_malloc(strlen(filename) + 1);
  strcpy(x->filename, filename);
  x->state = mpc_state_new();
  x->expected_num = 0;
  x->expected = NULL;
  x->failure = mpc_ctx_malloc(strlen(failure) + 1);
  strcpy(x->failure, failure);
  x->received = ' ';
  return x;
}
...
static void mpc_undefine_and(mpc_parser_t *p) {
  ...
  mpc_ctx_free(p->data.and.xs);
  mpc_ctx_free(p->data.and.dxs);
}
...
static void mpc_undefine_unretained(mpc_parser_t *p, int force) {
  ...
  case MPC_TYPE_FAIL:
    mpc_ctx_free(p->data.fail.m);
    break;

  case MPC_TYPE_ONEOF:
  case MPC_TYPE_NONEOF:
  case MPC_TYPE_STRING:
    mpc_ctx_free(p->data.string.x);
    break;
  ...
  case MPC_TYPE_CHECK:
    ...
    mpc_ctx_free(p->data.check.e);
    break;

  case MPC_TYPE_CHECK_WITH:
    mpc_undefine_unretained(p->data.check_with.x, 0);
    mpc_ctx_free(p->data.check_with.e);
    break;
...
static mpc_parser_t *mpc_undefined(void) {
  mpc_parser_t *p = mpc_ctx_calloc(1, sizeof(mpc_parser_t));
  p->retained = 0;
...
mpc_parser_t *mpc_new(const char *name) {
  ...
  p->retained = 1;
  p->name = mpc_ctx_realloc(p->name, strlen(name) + 1);
  strcpy(p->name, name);
  return p;
}
...
mpc_parser_t *mpc_copy(mpc_parser_t *a) {
  ...
  if (a->name) {
    p->name = mpc_ctx_malloc(strlen(a->name) + 1);
  ...
  case MPC_TYPE_FAIL:
    p->data.fail.m = mpc_ctx_malloc(strlen(a->data.fail.m) + 1);
  ...
  case MPC_TYPE_NONEOF:
  case MPC_TYPE_STRING:
    p->data.string.x = mpc_ctx_malloc(strlen(a->data.string.x) + 1);
  ...
  case MPC_TYPE_EXPECT:
    ...
    p->data.expect.m = mpc_ctx_malloc(strlen(a->data.expect.m) + 1);
  ...
  case MPC_TYPE_OR:
    p->data.or.xs = mpc_ctx_malloc(a->data.or.n * sizeof(mpc_parser_t *));
    for (i = 0; i < a->data.or.n; i++) {
      p->data.or.xs[i] = mpc_copy(a->data.or.xs[i]);
    }
    break;
  case MPC_TYPE_AND:
    p->data.and.xs = mpc_ctx_malloc(a->data.and.n * sizeof(mpc_parser_t *));
    for (i = 0; i < a->data.and.n; i++) {
      p->data.and.xs[i] = mpc_copy(a->data.and.xs[i]);
    }
    if (a->data.and.n > 0) {
      p->data.and.dxs =
          mpc_ctx_malloc((a->data.and.n - 1) * sizeof(mpc_dtor_t));
  ...
  case MPC_TYPE_CHECK:
    ...
    p->data.check.e = mpc_ctx_malloc(strlen(a->data.check.e) + 1);
    strcpy(p->data.check.e, a->data.check.e);
    break;
  case MPC_TYPE_CHECK_WITH:
    p->data.check_with.x = mpc_copy(a->data.check_with.x);
    p->data.check_with.e = mpc_ctx_malloc(strlen(a->data.check_with.e) + 1);
...
void mpc_cleanup(int n, ...) {
  int i;
  mpc_parser_t **list = mpc_ctx_malloc(sizeof(mpc_parser_t *) * n);
...
mpc_parser_t *mpc_fail(const char *m) {
  ...
  p->data.fail.m = mpc_ctx_malloc(strlen(m) + 1);
...
mpc_parser_t *mpc_failf(const char *fmt, ...) {
  ...
  buffer = mpc_ctx_realloc(buffer, strlen(buffer) + 1);
...
mpc_parser_t *mpc_expect(mpc_parser_t *a, const char *expected) {
  ...
  p->data.expect.x = a;
  p->data.expect.m = mpc_ctx_malloc(strlen(expected) + 1);
...
mpc_parser_t *mpc_oneof(const char *s) {
  ...
  p->data.string.x = mpc_ctx_malloc(strlen(s) + 1);
...
mpc_parser_t *mpc_check(mpc_parser_t *a, mpc_dtor_t da, mpc_check_t f,
  ...
  p->data.check.dx = da;
  p->data.check.f = f;
  p->data.check.e = mpc_ctx_malloc(strlen(e) + 1);
...
mpc_parser_t *mpc_check_with(mpc_parser_t *a, mpc_dtor_t da, mpc_check_with_t f,
  ...
  p->data.check_with.e = mpc_ctx_malloc(strlen(e) + 1);
...
mpc_parser_t *mpc_or(int n, ...) {
  ...
  p->type = MPC_TYPE_OR;
  p->data.or.n = n;
  p->data.or.xs = mpc_ctx_malloc(sizeof(mpc_parser_t *) * n);
...
mpc_parser_t *mpc_and(int n, mpc_fold_t f, ...) {
  ...
  p->data.and.n = n;
  p->data.and.f = f;
  p->data.and.xs = mpc_ctx_malloc(sizeof(mpc_parser_t *) * n);
  p->data.and.dxs = mpc_ctx_malloc(sizeof(mpc_dtor_t) * (n - 1));
...
static mpc_val_t *mpcf_re_range(mpc_val_t *x) {
  ...
  for (i = comp; i < strlen(s); i++) {
    ...
    if (s[i] == '\\') {
      ...
      if (tmp != NULL) {
        range = mpc_ctx_realloc(range, strlen(range) + strlen(tmp) + 1);
        strcat(range, tmp);
      } else {
        range = mpc_ctx_realloc(range, strlen(range) + 1 + 1);
    ...
    /* Regex Range...Range */
      ...
      if (s[i + 1] == '\0' || i == 0) {
        range = mpc_ctx_realloc(range, strlen(range) + strlen("-") + 1);
        strcat(range, "-");
      } else {
        start = s[i - 1] + 1;
        end = s[i + 1] - 1;
        for (j = start; j <= end; j++) {
          range = mpc_ctx_realloc(range, strlen(range) + 1 + 1 + 1);
          range[strlen(range) + 1] = '\0';
...
mpc_val_t *mpcf_ctor_str(void) { return mpc_ctx_calloc(1, 1); }
...
mpc_val_t *mpcf_int(mpc_val_t *x) {
  int *y = mpc_ctx_malloc(sizeof(int));
  *y = strtol(x, NULL, 10);
  mpc_ctx_free(x);
  return y;
}
...
mpc_val_t *mpcf_float(mpc_val_t *x) {
  float *y = mpc_ctx_malloc(sizeof(float));
  *y = strtod(x, NULL);
  mpc_ctx_free(x);
  return y;
}

static inline bool isspace_(unsigned char c) {
  return EQUAL_ANY(c, ' ', '\n', '\t', '\f');
}
...
mpc_val_t *mpcf_strtrimr(mpc_val_t *x) {
  char *s = x;
  size_t l = strlen(s);
  while (l > 0 && isspace_((unsigned char)s[l - 1])) {
    s[l - 1] = '\0';
    l--;
  }
  return s;
...
static mpc_val_t *mpcf_escape_new(mpc_val_t *x, const char *input,
  ...
  while (*s) {
    ...
    while (output[i]) {
      if (*s == input[i]) {
        y = mpc_ctx_realloc(y, strlen(y) + strlen(output[i]) + 1);
        strcat(y, output[i]);
        found = 1;
        break;
      }
    ...
    if (!found) {
      y = mpc_ctx_realloc(y, strlen(y) + 2);
      buff[0] = *s;
      buff[1] = '\0';
      strcat(y, buff);
    }
  ...
  while (*s) {
    ...
    while (output[i]) {
      if ((*(s + 0)) == output[i][0] && (*(s + 1)) == output[i][1]) {
        y = mpc_ctx_realloc(y, strlen(y) + 1 + 1);
        buff[0] = input[i];
        buff[1] = '\0';
        strcat(y, buff);
        found = 1;
...
void mpc_ast_delete(mpc_ast_t *a) {
  ...
  mpc_ctx_free(a->children);
  mpc_ctx_free(a->tag);
  mpc_ctx_free(a->contents);
  mpc_ctx_free(a);
}
...
mpc_ast_t *mpc_ast_new(const char *tag, const char *contents) {
  ...
  mpc_ast_t *a = mpc_ctx_malloc(sizeof(mpc_ast_t));

  a->tag = mpc_ctx_malloc(strlen(tag) + 1);
  strcpy(a->tag, tag);

  a->contents = mpc_ctx_malloc(strlen(contents) + 1);
...
mpc_ast_t *mpc_ast_add_child(mpc_ast_t *r, mpc_ast_t *a) {
  r->children_num++;
  r->children =
      mpc_ctx_realloc(r->children, sizeof(mpc_ast_t *) * r->children_num);
...
mpc_ast_t *mpc_ast_add_tag(mpc_ast_t *a, const char *t) {
  ...
  a->tag = mpc_ctx_realloc(a->tag, strlen(t) + 1 + strlen(a->tag) + 1);
...
mpc_ast_t *mpc_ast_tag(mpc_ast_t *a, const char *t) {
  a->tag = mpc_ctx_realloc(a->tag, strlen(t) + 1);
  strcpy(a->tag, t);
  return a;
}
...
mpc_ast_trav_t *mpc_ast_traverse_start(mpc_ast_t *ast,
  ...
  trav = mpc_ctx_malloc(sizeof(mpc_ast_trav_t));
  ...
  case mpc_ast_trav_order_post:
    while (cnode->children_num > 0) {
      ...
      n_trav = mpc_ctx_malloc(sizeof(mpc_ast_trav_t));
...
static mpc_parser_t *mpca_grammar_find_parser(char *x, mpca_grammar_st_t *st) {
  ...
  if (is_number(x)) {
    ...
    while (st->parsers_num <= i) {
      st->parsers_num++;
      st->parsers = mpc_ctx_realloc(st->parsers,
...
static mpc_val_t *mpca_stmt_afold(int n, mpc_val_t **xs) {
  mpca_stmt_t *stmt = mpc_ctx_malloc(sizeof(mpca_stmt_t));
  stmt->ident = ((char **)xs)[0];
  stmt->name = ((char **)xs)[1];
  stmt->grammar = ((mpc_parser_t **)xs)[3];
  (void)n;
  mpc_ctx_free(((char **)xs)[2]);
  mpc_ctx_free(((char **)xs)[4]);
...
static mpc_val_t *mpca_stmt_fold(int n, mpc_val_t **xs) {
  ...
  int i;
  mpca_stmt_t **stmts = mpc_ctx_malloc(sizeof(mpca_stmt_t *) * (n + 1));
...
static void mpca_stmt_list_delete(mpc_val_t *x) {
  ...
  while (*stmts) {
    ...
    mpc_ctx_free(stmt->ident);
    mpc_ctx_free(stmt->name);
...
static void mpc_optimise_unretained(mpc_parser_t *p, int force) {
  ...
  while (1) {
    ...
    if (p->type == MPC_TYPE_OR &&
      ...
      p->data.or.n = n + m - 1;
      p->data.or.xs =
          mpc_ctx_realloc(p->data.or.xs, sizeof(mpc_parser_t *) * (n + m - 1));
      memmove(p->data.or.xs + n - 1, t->data.or.xs, m * sizeof(mpc_parser_t *));
      mpc_ctx_free(t->data.or.xs);
      mpc_ctx_free(t->name);
      mpc_ctx_free(t);
      continue;
    }
    ...
    if (p->type == MPC_TYPE_AND && p->data.and.f == mpcf_fold_ast &&
      ...
      p->data.and.n = n + m - 1;
      p->data.and.xs =
          mpc_ctx_realloc(p->data.and.xs, sizeof(mpc_parser_t *) * (n + m - 1));
      p->data.and.dxs = mpc_ctx_realloc(p->data.and.dxs,
      ...
      mpc_ctx_free(t->data.and.xs);
      mpc_ctx_free(t->data.and.dxs);
      mpc_ctx_free(t->name);
      mpc_ctx_free(t);
      continue;
    }
